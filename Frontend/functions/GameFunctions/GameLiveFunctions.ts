...
export const goThroughEachGameAction = async (
    allTiles : [Tile], 
    actions : [any], 
    player1SelectedTiles: [SelectedTile], player2SelectedTiles: [SelectedTile],
    player1Depth: [Player], player2Depth: [Player],
    isPlayer1: boolean,
) => {
    ...
    actions.forEach((action) => {
        ...
        if(action.actionType === "timeout"){
            const swapTiles = (selectedTiles: [SelectedTile]) => {
                selectedTiles.forEach((selectedTile, index) => {
                    if(selectedTile.swapTile){
                        ...
                        selectedTiles[index] = {
                            ...swapTile,
                            name: swapTile.name,
                            goal: isPlayer1 ? swapTile.team1Goal : swapTile.team2Goal,
                            progress: isPlayer1 ? swapTile.team1Progress : swapTile.team2Progress,
                            index: swapTile.index,
                            row: swapTile.row,
                            tileIndex: swapTile.tileIndex,
                            team: isPlayer1 ? swapTile.team1 : swapTile.team2,
                            complete: isPlayer1 ? swapTile.team1Complete : swapTile.team2Complete,
                            swapTile: null
                        }
                    }
                }) 
                            ...
                            if(statObj){
                                ...
                                if(playerTileIndex === -1){
                                    player1Depth[playerIndex].tiles.push({
                                        ...
                                        progress: allTiles[selectedTile.tileIndex].team1Progress
                                    });
                                } else {
                ...
                player2SelectedTiles.forEach(selectedTile => {
                    if(!selectedTile.complete){
                        ...
                        selectedTileStats.forEach(selectedTileStat => {
                            ...
                            if(statObj){
                                /* Add to Tile */
                                const currTile = allTiles[selectedTile.tileIndex];
                                ...
                                if(playerTileIndex === -1){
                                    player2Depth[playerIndex].tiles.push({
                                        ...
                                        row: selectedTile.row,
                                        tileIndex: selectedTile.tileIndex,
                                        progress: allTiles[selectedTile.tileIndex].team2Progress
                                    });
                                } else {
